...
def _parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description=f"Project Manager CLI")
    parser.add_argument(
        "--repo", help=f"Local path to the repository's root directory.", default=f"./"
    )
    parser.add_argument(
        "--repo-upstream", help=f"Local path to the upstream repository's root directory."
    )
    parser.add_argument(
        "--metadata-ref", help=f"Git reference to read the metadata.json file from."
    )
    parser.add_argument("--metadata-filepath", help=f"Relative path to the metadata.json file.")
    parser.add_argument(
        "--main-metadata-filepath",
        help=f"Relative path to the metadata.json file in the default branch.",
    )
    parser.add_argument("--github-token", help=f"GitHub token for accessing the repository.")
    parser.add_argument(
        "--remove-tokens",
    ...
    subparsers_main = parser.add_subparsers(dest=f"command", required=True)
    subparser_cca = subparsers_main.add_parser(
        "cca", help=f"Run Continuous Configuration Automation on the repository."
    )
    ...
    subparser_lint = subparsers_main.add_parser(
        "lint", help=f"Run pre-commit hooks on the repository."
    )
    ...
    subparser_lint_mutually_exclusive_file.add_argument(
        "-a", "--all-files", help=f"Run on all files in the repository.", action=f"store_true"
    )
    subparser_lint_mutually_exclusive_file.add_argument(
        "-f", "--files", help=f"Run on specific files.", nargs=f"+"
    )
    subparser_lint_mutually_exclusive_file.add_argument(
        "-r1",
        "--from-ref",
    ...
    subparser_version = subparsers_main.add_parser(
        "version", help=f"Print the current version of the project."
    )
    subparser_version.set_defaults(endpoint="version.run_cli")
    subparser_build = subparsers_main.add_parser("build", help=f"Build project components.")
    # Sub-parsers for subparser_build
    subparsers_build = subparser_build.add_subparsers(dest=f"build", required=True)
    subparser_conda = subparsers_build.add_parser(
        "conda", help=f"Build a conda package in the project."
    )
    ...
    subparser_python = subparsers_build.add_parser(
        "python", help=f"Build a Python package in the project."
    )
    ...
    subparser_render = subparsers_main.add_parser(
        "render", help=f"Render documents in the project."
    )
    # Sub-parsers for subparser_render
    subparsers_render = subparser_render.add_subparsers(dest=f"render", required=True)
    subparser_pypi = subparsers_render.add_parser(
        "pypi", help=f"Render package README file for PyPI."
    )
    ...
    subparser_gha = subparsers_main.add_parser(
        "gha", help=f"Run CI/CD pipelines in GitHub Actions."
    )
