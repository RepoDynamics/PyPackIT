...
- Referencing the issue in other parts of the control center
  (see [Issue Assignees](maintainer.md#issue-assignees)).
...
which must match the key of one of the [primary type labels](label.md#primary-types),
and consequently, the key of either one of the [primary action commit types](commit.md#primary-action-types),
or one of the [primary custom commit types](commit.md#primary-custom-types).
...
The subtype must match the key of one of the [subtype labels](label.md#subtypes).
...
as defined in the [label configurations](label.md).

Note that the [primary type](label.md#primary-types), [subtype](label.md#subtypes),
and [status](label.md#statuses) labels are automatically added to each issue,
...
- Each form must have a `dropdown` element whose `id` is set to either `version` or `branch`,
  ...
  and automatically adds the corresponding [branch labels](label.md#branches)
  to each issue that is created using the form.
  ...
  [version labels](label.md#versions) and branch labels to each issue.
  ...
  Subsequently, when the issue is labeled as [in development](label.md#statuses),
