...
class ElementCollection:
    def __init__(self, elements: list = None, seperator: dict | list | str = '\n'):
        self.elements = (list(elements) if isinstance(elements, (list, tuple)) else [elements]) if elements else list()
...
class Element:
    ...
    def __init__(
        ...
        self.content = content if isinstance(content, ElementCollection) else ElementCollection(content)
    ...
    def __str__(self):
        ...
        content = '\n'.join([f'{self.content_indent}{line}' for line in str(self.content).split('\n')])
...
class DIV(Element):
    ...
    def __init__(
        ...
        super().__init__(tag='div', attrs=attrs, content=content, tag_seperator=tag_seperator, content_indent=content_indent)
        return
...
class P(Element):
    ...
    def __init__(
        ...
        super().__init__(
            tag='p', attrs=attrs, content=content, tag_seperator=tag_seperator, content_indent=content_indent
        )
        return
    ...
    def style(self, words: dict[str, dict[str, str | bool]], ignore_case=False):
        ...
        for word, config in words.items():
            ...
            if config.get('link'):
                mod_word = str(A(href=config.get('link'), content=[mod_word], tag_seperator='', content_indent=''))
        ...
        replaced_text = pattern.sub(lambda match: subs[normalize_old(match.group(0))], string, count=1)
        return P(content=[replaced_text], tag_seperator=self.tag_seperator, content_indent=self.content_indent, **self.attrs)
...
class A(Element):
    ...
    def __init__(
        ...
        """
        attrs['href'] = href
        super().__init__(tag='a', attrs=attrs, content=content, tag_seperator=tag_seperator, content_indent=content_indent)
        return
...
class PICTURE(Element):
    ...
    def __init__(
        ...
        super().__init__(tag='picture', attrs=attrs, content=ElementCollection([*sources, img], seperator=tag_seperator), tag_seperator=tag_seperator, content_indent=content_indent)
        return
